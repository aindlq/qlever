...
class PositionedReader {
 ...
 public:
  ...
  // Read up to `count` bytes at `offset` via the positioned-`ReadFile` handle
  // pool. Its handles are derived from `fd`, not from the name, so reads keep
  // working when the file has been renamed. `filename` is unused on Windows.
