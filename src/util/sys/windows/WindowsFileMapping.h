...
#ifndef QLEVER_SRC_UTIL_SYS_WINDOWS_WINDOWSFILEMAPPING_H
#define QLEVER_SRC_UTIL_SYS_WINDOWS_WINDOWSFILEMAPPING_H
...
#include "util/sys/windows/FileCompat.h"  // for the `truncate` shim
...
// `util/sys/FileMapping.h` for the compile-time seam selecting between the two.
class FileMapping {
 private:
 ...
 public:
  // Persist the mapped data and release the mapping. Windows cannot resize a
  // file that still has an active mapping (ERROR_USER_MAPPED_FILE), so the view
  // is released first; unmapping also flushes the written data.
...
#endif  // _WIN32
#endif  // QLEVER_SRC_UTIL_SYS_WINDOWS_WINDOWSFILEMAPPING_H
