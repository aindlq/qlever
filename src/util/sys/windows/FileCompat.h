...
// (`util/sys/windows/WindowsPositionedReader.h`), `openWithPosixSharing` and
...
inline FILE* openWithPosixSharing(const std::filesystem::path& filename,
  ...
  switch (primary) {
    ...
    case 'w':
      ...
      // so the `CreateFileW` below can recreate the name right away. A plain
      // `DeleteFileW` here would only mark the file delete-pending while it is
...
// POSIX name so that call sites stay portable: they pass
// `std::filesystem::path::c_str()`, which is `const char*` on POSIX (picking
// the real `truncate`) and `const wchar_t*` here (wide, so non-ASCII paths
// work).
inline int truncate(const wchar_t* path, int64_t length) {
  ...
  int fd = _wopen(path, _O_RDWR | _O_BINARY);
  if (fd < 0) return -1;
