...
#ifndef QLEVER_SRC_UTIL_WINDOWS_WINDOWSPOSITIONEDREADER_H
#define QLEVER_SRC_UTIL_WINDOWS_WINDOWSPOSITIONEDREADER_H
...
#include "util/windows/WindowsUndefs.h"  // for windows.h (HANDLE, ReadFile, ...)
...
class PositionedReadHandle {
 ...
 public:
  ...
  // (like POSIX `pread`).
  ssize_t pread(const std::string& filename, void* buffer, size_t count,
                off_t offset) {
 ...
 private:
  ...
  HANDLE getOrOpen(const std::string& filename) {
  ...
  static HANDLE openHandle(const std::string& filename) {
    HANDLE fresh =
...
// `pread`- emulating `PositionedReadHandle` pool. See `util/PositionedReader.h`
// for the compile-time seam selecting the impl.
...
#endif  // _WIN32
#endif  // QLEVER_SRC_UTIL_WINDOWS_WINDOWSPOSITIONEDREADER_H
