...
#ifndef QLEVER_SRC_UTIL_WINDOWS_WINDOWSUNDEFS_H
#define QLEVER_SRC_UTIL_WINDOWS_WINDOWSUNDEFS_H
...
// collides with QLever's own large-file-safe, mmap-aware `truncate(const char*,
// int64_t)` shim in `FileCompat.h`, making the single call site in
// `MmapVectorImpl.h` ambiguous. (Under _FILE_OFFSET_BITS=64 mingw remaps
// `ftruncate`->`ftruncate64` but, by a quirk, leaves `truncate` 32-bit.)
...
#endif  // QLEVER_SRC_UTIL_WINDOWS_WINDOWSUNDEFS_H
