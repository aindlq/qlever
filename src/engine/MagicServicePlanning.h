...
class MagicServicePlanningContext {
 public:
  ...
  // call `makeOperation(child)` to build the service's operation, and add it as a
  // candidate. Use this for services whose operation wraps a nested pattern.
...
using MagicServicePlanner =
    std::function<void(MagicServicePlanningContext&,
                       parsedQuery::MagicServiceQuery&)>;
