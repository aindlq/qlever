...
// `EagerInitializedSharedMutex` on Windows. Its constructor performs the lazy
// initialization before the mutex can be shared between threads.
//
// This file contains:
//  * GREEN regression guards (run everywhere) that
//    `EagerInitializedSharedMutex` and the `Synchronized` *default* mutex
//    preserve mutual exclusion, also under cold concurrent first use. These
//    would turn RED on Windows if the default were reverted to
//    `std::shared_mutex`.
...
#include "util/sys/windows/EagerInitializedSharedMutex.h"

using ad_utility::EagerInitializedSharedMutex;
...
// ---------------------------------------------------------------------------
// EagerInitializedSharedMutex: basic correctness (green on every platform)
// ---------------------------------------------------------------------------

TEST(EagerInitializedSharedMutex, ExclusiveLockProvidesMutualExclusion) {
  EagerInitializedSharedMutex m;
...
TEST(EagerInitializedSharedMutex, SharedLockAllowsConcurrentReaders) {
  EagerInitializedSharedMutex m;
...
TEST(EagerInitializedSharedMutex, ExclusiveLockExcludesSharedLocks) {
  EagerInitializedSharedMutex m;
...
// `EagerInitializedSharedMutex` initializes its underlying mutex before
// construction completes, so its first concurrent use is no longer cold.
TEST(EagerInitializedSharedMutex, ColdStartMutualExclusionStress) {
  EXPECT_EQ(coldStartViolations<EagerInitializedSharedMutex>(1500), 0);
}

// Guard the abstraction at the level QLever actually uses: a freshly
// constructed `Synchronized` locked from several threads at once. With the
// Windows default reverted to `std::shared_mutex` this loses updates; with
// `EagerInitializedSharedMutex` it must not. Detects both overlap (sentinel)
// and lost updates (counter).
...
// `EagerInitializedSharedMutex` does NOT cover.
//
// DISABLED because it passes on glibc (no bug) and fails only on MinGW. To
// reproduce on Windows:
//   ./EagerInitializedSharedMutexTest --gtest_also_run_disabled_tests
...
TEST(SharedMutexColdStartRace, DISABLED_StdSharedMutexColdStartMingwBug883) {
  ...
  EXPECT_EQ(violations, 0)
      ...
      << " mutual-exclusion violations on a freshly-constructed "
         ...
         "same exposure and is not covered by EagerInitializedSharedMutex.";
}
